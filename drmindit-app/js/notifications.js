/* ============================================================
   DRMINDIT NOTIFICATION SERVICE
   Handles browser push permissions and local triggers.
   ============================================================ */

const DrMinditNotifications = {
    settings: {
        enabled: true,
        reminderTime: '08:00',
    ...
    // ── INITIALISE ──
    async init() {
        console.log('✦ DrMindit Notifications: Initializing...');

        // Load settings from Supabase if authenticated
        if (window.DrMinditAuth && DrMinditAuth.user) {
            await this.loadSettings(DrMinditAuth.user.id);
        }

        // Check permission state
        if ('Notification' in window) {
            console.log('✦ DrMindit Notifications: Permission status:', Notification.permission);
        }
    },
    ...
    async requestPermission() {
        if (!('Notification' in window)) {
            console.warn('✦ DrMindit Notifications: Browser does not support desktop notifications.');
            return false;
        }

        const permission = await Notification.requestPermission();
        if (permission === 'granted') {
            console.log('✦ DrMindit Notifications: Permission granted.');
            this.sendLocalNotification('Notifications Enabled', {
                body: 'You will now receive daily reminders for your DrMindit journey.'
            });
            return true;
        }
        return false;
    ...
    async loadSettings(userId) {
        ...
        if (error && error.code !== 'PGRST116') { // PGRST116 is "no rows found"
            console.error('✦ DrMindit Notifications: Error loading settings:', error.message);
            return;
        }
        ...
        if (data) {
            ...
            console.log('✦ DrMindit Notifications: Settings loaded.');
        } else {
    ...
    async saveSettings(updates) {
        if (!window.DrMinditAuth?.user) return;

        const userId = DrMinditAuth.user.id;
        const payload = {
            user_id: userId,
            ...updates,
        ...
        if (error) {
            console.error('✦ DrMindit Notifications: Error saving settings:', error.message);
        } else {
            console.log('✦ DrMindit Notifications: Settings saved.');
            // Update local cache
    ...
    sendLocalNotification(title, options = {}) {
        ...
        if (Notification.permission !== 'granted') {
            console.warn('✦ DrMindit Notifications: Permission not granted for push.');
            return;
        }
        ...
        n.onclick = (e) => {
            ...
            if (window.DrMinditState) {
                DrMinditState.navigateTo('resilience');
            }
        };
    },

    async logNotification(type, channel, message) {
        if (!window.DrMinditSupabase || !window.DrMinditAuth?.user) return;

        await window.DrMinditSupabase
            .from('notification_logs')
            .insert({
                user_id: DrMinditAuth.user.id,
    ...
    async checkAndNotify() {
        ...
        const currentDay = window.DrMinditState ? DrMinditState.resilienceDay : 1;
        const progress = window.DrMinditState ? DrMinditState.resilienceProgress : {};
        ...
        if (!isCompletedToday) {
            ...
            if (hour >= reminderTimeHour) {
                ...
                if (hour >= 19) { // Evening catch-up
                    message = "Take a few minutes tonight to complete your DrMindit session.";
                } else {
                    ...
                    const templates = [
                        `Day ${currentDay} of your DrMindit journey is ready.`,
                        `Time to breathe and reset. Continue your 21-day DrMindit journey.`,
                        `Your DrMindit session is ready. Take ${duration} minutes to reset your mind.`
                    ];
    ...
    triggerStreakMotivation(streak) {
        ...
        if (streak === 3) {
            ...
            message = "Congratulations! You completed the DrMindit program.";
        } else if (streak > 1) {
            const messages = [
                `Amazing! You're on a ${streak}-day DrMindit streak.`,
    ...
    triggerRecoveryNotification(missedDays) {
        ...
        if (message) {
            this.sendLocalNotification('DrMindit Recovery', { body: message });
